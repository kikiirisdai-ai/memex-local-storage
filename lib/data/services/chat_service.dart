...
@visibleForTesting
bool shouldSkipQuickCapture(String runMode) => runMode == 'agent';

@visibleForTesting
bool shouldForceCard(String runMode) => runMode == 'card';
...
class ChatService {
  ...
  Stream<ChatEvent> sendMessage(
    ...
    // Persist the voice-note audio (if any) as an asset so the resulting
    // timeline card can reference and play it back.
    String? audioFsFilename;
    if (audioPath != null && audioPath.isNotEmpty) {
      try {
        final (fsName, _) = await _fileService.saveAssetFromFile(
          userId: userId,
          sourcePath: audioPath,
          assetType: 'audio',
        );
        audioFsFilename = fsName;
      } catch (e) {
        _logger.warning('Failed to save voice note audio asset: $e');
      }
    }
    ...
    try {
      ...
      final taskId = await LocalTaskExecutor.instance.enqueueTask(
        ...
        payload: {
          ...
          'audio_fs_filename': audioFsFilename,
          'is_quick_query': isQuickQuery,
          'run_mode': runMode,
          'original_text': originalText,
          'polished_style': polishedStyle,
  ...
  Future<void> handleSuperAgentChatTurnTask(
    ...
    final audioFsFilename = payload['audio_fs_filename'] as String?;
    ...
    final polishedStyle = payload['polished_style'] as String?;
    final verbatimText = payload['verbatim_text'] as bool? ?? false;
  ...
  /// Attempts the quick-capture fast path. Returns true when the turn was
  /// fully handled (card written and/or reply persisted, stream finalized);
  /// false means the caller must continue with the full pipeline.
  Future<bool> _tryQuickCapture({
    ...
    final inlineImages = <InlineAgentImage>[];
    final imageFsFilenames = <String>[];
    for (final attachment in preparedImages) {
      final image = await inlineImageForLlm(
        absolutePath:
            FileSystemService.instance.toAbsolutePath(attachment.relativePath),
        fallbackMimeType: attachment.mimeType,
        logLabel: attachment.fsUri,
      );
      if (image == null) {
        // Image cannot be inlined; the full pipeline has richer handling.
        return false;
      }
      inlineImages.add(image);
      imageFsFilenames.add(attachment.fsFilename);
    }

    final result = await QuickCaptureService.instance.run(
      userId: userId,
      message: message,
      images: inlineImages,
    ...
    if (result.outcome == QuickCaptureOutcome.escalate) {
      return false;
    }

    final replyText = (result.replyText?.trim().isNotEmpty ?? false)
        ? result.replyText!.trim()
        : (result.outcome == QuickCaptureOutcome.card ? '✓' : '');

    run.add(ChatAgentStartedEvent(turnId));
    if (replyText.isNotEmpty) {
      run.add(ChatResponseChunkEvent(turnId, replyText));
      if (!await _sessionHasAssistantForTurn(userId, sessionId, turnId)) {
        await _addMessageToSession(
          userId,
          sessionId,
    ...
    if (result.goalSuggestions.isNotEmpty) {
      final artifacts = result.goalSuggestions
          .map((s) => ChatArtifact.goalSuggestion(
                goalId: s.goalId,
    ...
    run.add(ChatResponseChunkEvent(turnId, '', isDone: true));
    run.add(ChatAgentStoppedEvent(turnId));
    if (await _shouldCloseRunAfterTask(taskId)) {
      run.close();
    }
    _logger.info(
      'Quick capture handled turn $turnId (${result.outcome.name}'
      '${result.cardFactId != null ? ', card ${result.cardFactId}' : ''})',
    );
    return true;
  }
  ...
  Future<void> _runSuperAgentChatTurn({
    ...
    try {
      ...
      // Fast path: simple fragments are handled with a single LLM call
      // (record → card, chit-chat → reply). Escalation or any failure falls
      // through to the unchanged full pipeline below.
      final forceCard = shouldForceCard(runMode);
      if (customAgentCfg == null &&
          !shouldSkipQuickCapture(runMode) &&
          (forceCard ||
              QuickCaptureService.isEligible(
                scene: scene,
                agentName: agentName,
                message: message,
                refs: refs,
        ...
        final handled = await _tryQuickCapture(
          userId: userId,
          sessionId: sessionId,
          turnId: turnId,
          taskId: taskId,
