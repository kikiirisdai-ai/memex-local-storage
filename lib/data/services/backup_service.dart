...
/// Merges snapshot lists gathered from multiple storage locations (default
/// backup directory, safety-snapshot directory, Android SAF tree, ...) into
/// a single de-duplicated, newest-first list.
///
/// De-duplication is by normalized [BackupSnapshot.filePath] (case-sensitive,
/// matching filesystem semantics on the platforms this app targets): if two
/// groups happen to report the same file path (e.g. the default and safety
/// directories were ever configured to coincide), only the first occurrence
/// is kept. Snapshots without a [BackupSnapshot.filePath] (e.g. Android
/// SAF/document-backed entries, which are identified by [BackupSnapshot.id]
/// instead) are never deduped against each other by path and are always
/// kept.
List<BackupSnapshot> mergeAndDedupeSnapshots(
  List<List<BackupSnapshot>> groups,
) {
  final merged = <BackupSnapshot>[];
...
class BackupService {
  ...
  static Future<bool> restoreStoredBackup(
    ...
    if (snapshot.isICloud) {
      sourcePath = await ExternalBackupChannel().readFileToTemp(snapshot.name);
      deleteTempAfterRestore = true;
    }
  ...
  static Future<void> deleteStoredBackup(BackupSnapshot snapshot) async {
    ...
    if (snapshot.isICloud) {
      await ExternalBackupChannel().deleteFile(snapshot.name);
      _emitBackupSnapshotsChanged('deleted', snapshotId: snapshot.id);
      return;
    }
  ...
  /// List automatic/safety backups from the platform default directory, the
  /// out-of-dataRoot safety-snapshot directory (see
  /// [resolveSafetySnapshotDirectory]), and the user's Android SAF directory,
  /// if configured.
  static Future<List<BackupSnapshot>> listStoredBackups() async {
    final groups = <List<BackupSnapshot>>[];

    try {
      final defaultDir = await resolveDefaultBackupDirectory();
      groups.add(await _listFileBackups(defaultDir));
    } catch (e, st) {
      _logger.warning('Failed to list default backups: $e', e, st);
    }

    try {
      final safetyDir = await resolveSafetySnapshotDirectory();
      groups.add(await _listFileBackups(safetyDir));
    } catch (e, st) {
      _logger.warning('Failed to list safety-snapshot backups: $e', e, st);
    }
    ...
    if (Platform.isAndroid && userId != null && userId.isNotEmpty) {
      ...
      if (treeUri != null && treeUri.isNotEmpty) {
        try {
          groups.add(await _listAndroidTreeBackups(treeUri));
        } catch (e, st) {
    ...
    if (Platform.isIOS) {
      try {
        final files = await ExternalBackupChannel().listFiles();
        groups.add(files
            .map((f) => BackupSnapshot(
                  id: f.path,
                  name: f.name,
        ...
        _logger.warning('Failed to list iCloud backups: $e', e, st);
      }
    }

    return mergeAndDedupeSnapshots(groups);
  }

  /// Resolves the directory the pre-wipe safety snapshot is written into,
  /// guaranteed (with a defensive fallback) to be outside
  /// [FileSystemService.instance.dataRoot] — which a "clear all data" wipe
  /// recursively deletes. This is the single source of truth for that
  /// location: both the writer (see the delete-account flow in
  /// `settings_page.dart`) and [listStoredBackups] must resolve the same
  /// directory, or a written snapshot could become unreachable in the UI.
  static Future<Directory> resolveSafetySnapshotDirectory() async {
    final dataRoot = FileSystemService.instance.dataRoot;
    final supportDir = await getApplicationSupportDirectory();
    var safeDir = path.join(supportDir.path, 'SafetySnapshots');

    if (isPathInside(safeDir, dataRoot)) {
      // Should not happen in practice (app support dir is a sibling of the
      // documents directory dataRoot lives under), but fall back to the
      // temp directory defensively rather than risk writing inside dataRoot.
      final tempDir = await getTemporaryDirectory();
      safeDir = path.join(tempDir.path, 'SafetySnapshots');
    }

    final dir = Directory(safeDir);
    if (!await dir.exists()) {
