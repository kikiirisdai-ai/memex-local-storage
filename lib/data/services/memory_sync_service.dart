...
/// Text worth folding into long-term memory for [card].
///
/// Most cards carry their source-of-truth text in `card.fact`. Structured
/// cards (media/task/etc. captured with no free-text `fact`) carry their
/// meaning in `uiConfigs` instead — summarize those so a card's memory
/// isn't silently dropped just because it has no `fact`. Returns null when
/// there's genuinely nothing worth syncing.
@visibleForTesting
String? deriveMemorySyncText(CardData card) {
  final directFact = card.fact?.trim();
  if (directFact != null && directFact.isNotEmpty) return directFact;
  ...
  for (final config in card.uiConfigs) {
    final fields = config.data.entries
        .where((e) => e.value != null && e.value.toString().trim().isNotEmpty)
        .map((e) => '${e.key}: ${e.value}')
        .join(', ');
    if (fields.isNotEmpty) {
      parts.add('[${config.templateId}] $fields');
    }
  }
...
/// Where a user's long-term memory stands relative to their captured cards.
enum MemorySyncStatus {
  /// A sync batch is running right now.
  updating,

  /// Cards have been captured (or a manual re-check found stragglers) that
  /// haven't been folded into memory yet.
  pendingUpdate,

  /// Nothing pending — memory reflects every captured card.
  upToDate,
}
...
class MemorySyncService {
  ...
  Future<_UserSyncState> _ensureLoaded(String userId) async {
  ...
  Future<void> enqueueFact(String userId, String factId) async {
    final state = await _ensureLoaded(userId);
  ...
  /// Enqueues several fact IDs at once (e.g. a historical backfill scan),
  /// without triggering processing — callers decide when to sync.
  Future<void> enqueueMany(String userId, Iterable<String> factIds) async {
    final state = await _ensureLoaded(userId);
    state.pendingFactIds.addAll(factIds);
    await _savePendingFactIds(userId);
  }

  /// Current sync status for [userId], for UI display.
  Future<MemorySyncStatus> getStatus(String userId) async {
    final state = await _ensureLoaded(userId);
    if (state.isProcessing) return MemorySyncStatus.updating;
    if (state.pendingFactIds.isNotEmpty) return MemorySyncStatus.pendingUpdate;
    return MemorySyncStatus.upToDate;
  }

  /// Forces the pending queue to process right now, regardless of the
  /// batch threshold. Returns the number of facts processed (0 if nothing
  /// was pending or a sync was already running).
  Future<int> processNow(String userId) => _processQueue(userId);

  /// Processes pending facts in the queue asynchronously.
  /// Handles concurrency using `isProcessing` flag per user.
  Future<int> _processQueue(String userId) async {
    final state = _getState(userId);
    ...
    try {
      ...
      while (state.pendingFactIds.isNotEmpty) {
        ...
        for (final factId in batch) {
          ...
          if (card == null) {
            _logger.warning('Card not found, skipping: $factId');
            continue;
          }
          final factText = deriveMemorySyncText(card);
          if (factText == null || factText.isEmpty) {
            _logger.warning('No syncable content for: $factId');
            continue;
          }
      ...
      // The agent-activity bubble above is transient (visible only while
      // this runs); surface a toast too so a completed sync isn't missed
      // just because no one was watching the home screen.
      if (processedCount > 0) {
        EventBusService.instance.emitEvent(
          MemorySyncCompletedMessage(processedCount: processedCount),
        );
      }
