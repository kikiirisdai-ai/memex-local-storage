...
class UserStatsService {
  ...
  Future<void> _collectCardStats(
    ...
    for (final cardPath in cardPaths) {
      ...
      final mood = _effectiveMoodScore(card);
      if (mood != null) {
        bucket.moodSum += mood;
        bucket.moodCount += 1;
      }
  ...
  /// Effective per-card mood (1-10), computed deterministically without any
  /// LLM. The user's own rating always wins over the model-inferred one.
  int? _effectiveMoodScore(CardData card) {
    return sanitizeMoodScore(card.metadata?[CardMetadataKeys.userMoodScore]) ??
...
class _DailyStatsAccumulator {
  ...
  UserStatsDailyPoint toPoint() => UserStatsDailyPoint(
        ...
        moodAverage: moodCount == 0 ? null : moodSum / moodCount,
      );
}
