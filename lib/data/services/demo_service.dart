...
class DemoService extends ChangeNotifier {
  ...
  void advance() {
    ...
    if (idx < steps.length - 1) {
      ...
      // The Insight tab chip that used to host this step's spotlight was
      // removed from the home timeline, so there's no tap target left —
      // skip straight through to the next step.
      if (_currentStep == DemoStep.tapInsightTab) {
        advance();
        return;
      }
      notifyListeners();
