...
class ChatArtifact {
  ...
  static const String kindGoalSuggestion = 'goal_suggestion';
  ...
  /// A pre-existing item merely being cited as a source for an answer
  /// (e.g. `search_cards` results), not created/updated/saved by this turn.
  ...
  String? get goalSuggestionGoalId => _nonEmpty(metadata['goal_id']);
  String? get goalSuggestionTitle => _nonEmpty(metadata['goal_title']);
  String? get goalSuggestionGoalType => _nonEmpty(metadata['goal_type']);
  double? get goalSuggestionDelta {
    final raw = metadata['delta'];
  ...
  factory ChatArtifact.timelineCard({
    ...
    return ChatArtifact(
      ...
      operation: operation ?? (updated ? operationUpdate : operationCreate),
  ...
  /// A quick-capture suggestion to nudge an in-progress goal's value.
  /// `operation` is always [operationReference] — this is a proposal, not
  /// something the turn created/updated. Resolution (confirmed or not) is
  /// tracked separately via [GoalSuggestionResolutionStore], keyed by
  /// [artifactId] — never mutated on this object.
  factory ChatArtifact.goalSuggestion({
    ...
    final targetUri = 'memex://goal-suggestion/'
        '${Uri.encodeComponent(goalId)}/${Uri.encodeComponent(turnId)}';
    return ChatArtifact(
      artifactId: artifactIdFor(kindGoalSuggestion, targetUri),
      kind: kindGoalSuggestion,
      operation: operationReference,
      title: _nonEmpty(goalTitle),
      targetUri: targetUri,
      sourceRunId: turnId,
      createdAt: createdAt,
      metadata: {
        ...
        'goal_type': goalType,
        if (delta != null && goalType != 'binary') 'delta': delta,
      },
    );
  }
