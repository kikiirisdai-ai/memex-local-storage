...
/// Update the user's own mood rating for a card (1-10).
/// Stored under [CardMetadataKeys.userMoodScore]; overrides the AI score.
///
/// Returns:
///   bool: success
Future<bool> updateCardMoodEndpoint(String cardId, int score) async {
  _logger.info('updateCardMood called: cardId=$cardId, score=$score');
  if (sanitizeMoodScore(score) == null) {
    _logger.warning('Rejected out-of-range mood score $score for $cardId');
    return false;
  }
  ...
  try {
    final userId = await UserStorage.getUserId();
    if (userId == null) {
      throw Exception('User not logged in, cannot update card mood');
    }
    ...
    final updatedCardData = await _fileSystemService.updateCardFile(
      ...
      (card) => card.copyWith(metadata: {
        ...?card.metadata,
        CardMetadataKeys.userMoodScore: score,
      }),
    );
    ...
    _logger.info('Updated user_mood_score for card $cardId to $score');
    return true;
  } catch (e) {
...
/// Update the independent media status ('want'|'doing'|'done') for a card.
/// Stored under [CardMetadataKeys.mediaStatus]. Does not touch rating.
///
/// Returns:
///   bool: success
Future<bool> updateCardMediaStatusEndpoint(String cardId, String status) async {
  _logger.info('updateCardMediaStatus called: cardId=$cardId, status=$status');
  final sanitized = sanitizeMediaStatus(status);
  if (sanitized == null) {
    _logger.warning('Rejected invalid media status "$status" for $cardId');
    return false;
  }
  ...
  try {
    final userId = await UserStorage.getUserId();
    if (userId == null) {
      throw Exception('User not logged in, cannot update media status');
    }
    ...
    final updatedCardData = await _fileSystemService.updateCardFile(
      ...
      (card) => card.copyWith(metadata: {
        ...?card.metadata,
        CardMetadataKeys.mediaStatus: sanitized,
      }),
    );
    ...
    _logger.info('Updated media_status for card $cardId to $sanitized');
    return true;
  } catch (e) {
...
/// Update the user's own media rating (1-10) and short comment for a card.
/// Stored under [CardMetadataKeys.userMediaRating] (overrides the
/// AI-inferred rating) and [CardMetadataKeys.mediaComment] (overwritten in
/// place; a null/blank [comment] clears the field).
///
/// Returns:
///   bool: success
Future<bool> updateCardMediaRatingEndpoint(
  String cardId,
  int rating,
  String? comment,
) async {
  _logger.info(
    'updateCardMediaRating called: cardId=$cardId, rating=$rating',
  );
  if (sanitizeMoodScore(rating) == null) {
    _logger.warning('Rejected out-of-range media rating $rating for $cardId');
    return false;
  }
  ...
  try {
    final userId = await UserStorage.getUserId();
    if (userId == null) {
      throw Exception('User not logged in, cannot update media rating');
    }
    ...
    final updatedCardData = await _fileSystemService.updateCardFile(
      ...
      (card) {
        ...
        metadata[CardMetadataKeys.userMediaRating] = rating;
        if (trimmedComment != null && trimmedComment.isNotEmpty) {
          metadata[CardMetadataKeys.mediaComment] = trimmedComment;
        } else {
    ...
    _logger.info('Updated media rating for card $cardId to $rating');
    return true;
  } catch (e) {
