...
class CardDetailModel {
  ...
  /// Structured machine fields copied from [CardData.metadata].
  final Map<String, dynamic>? metadata;

  /// Whether the user rated this card themselves (overrides the AI score).
  bool get isUserRatedMood =>
      sanitizeMoodScore(metadata?[CardMetadataKeys.userMoodScore]) != null;

  /// Validated mood score (1-10): the user's own rating when present,
  /// otherwise the model-inferred one. Null when the card carries no mood.
  int? get moodScore =>
      sanitizeMoodScore(metadata?[CardMetadataKeys.userMoodScore]) ??
      sanitizeMoodScore(metadata?[CardMetadataKeys.moodScore]);

  /// AI mood word from metadata. Hidden once the user rated the card, so a
  /// stale model label never contradicts the user's own score.
  String? get moodLabel {
    if (isUserRatedMood) return null;
    final label = metadata?[CardMetadataKeys.moodLabel];
    return label is String && label.trim().isNotEmpty ? label.trim() : null;
  }

  /// Validated media type ('book'|'movie'|'tv'|'music'|'podcast'|'other').
  /// Null means this card is not a media record.
  String? get mediaType => sanitizeMediaType(metadata?[CardMetadataKeys.mediaType]);

  /// Validated media status ('want'|'doing'|'done').
  String? get mediaStatusValue =>
      sanitizeMediaStatus(metadata?[CardMetadataKeys.mediaStatus]);

  /// Whether the user rated this media card themselves.
  bool get isUserRatedMedia =>
      sanitizeMoodScore(metadata?[CardMetadataKeys.userMediaRating]) != null;

  /// Validated rating (1-10): the user's own when present, otherwise the
  /// model-inferred one.
  int? get mediaRatingValue =>
      sanitizeMoodScore(metadata?[CardMetadataKeys.userMediaRating]) ??
      sanitizeMoodScore(metadata?[CardMetadataKeys.mediaRating]);

  String? get mediaComment => _trimmedOrNull(metadata?[CardMetadataKeys.mediaComment]);
  String? get mediaAuthor => _trimmedOrNull(metadata?[CardMetadataKeys.mediaAuthor]);
  String? get mediaYear => _trimmedOrNull(metadata?[CardMetadataKeys.mediaYear]);
  ...
  /// Type-safe: a non-String metadata value yields null instead of throwing.
  String? get originalText {
    final raw = metadata?[CardMetadataKeys.originalText];
    if (raw is! String) return null;
    final v = raw.trim();
    return v.isEmpty ? null : v;
  }

  /// Polish style: 'plain', 'literary', etc. Null if not present or not a
  /// String (type-safe read, mirrors [moodLabel]/[originalText]).
  String? get polishedStyle {
    final raw = metadata?[CardMetadataKeys.polishedStyle];
