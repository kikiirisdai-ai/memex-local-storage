...
enum EventBusMessageType {
  ...
  memorySyncCompleted('memory_sync_completed'),
  unknown('unknown');
...
abstract class EventBusMessage {
  ...
  factory EventBusMessage.fromJson(Map<String, dynamic> json) {
    ...
    switch (type) {
      ...
      case EventBusMessageType.memorySyncCompleted:
        return MemorySyncCompletedMessage.fromJson(json);
      default:
...
/// Emitted after a batch of facts has been folded into long-term memory,
/// so the UI can surface a toast even when no screen is watching the
/// transient agent-activity indicator.
class MemorySyncCompletedMessage extends EventBusMessage {
  final int processedCount;

  MemorySyncCompletedMessage({required this.processedCount})
    : super(
        type: EventBusMessageType.memorySyncCompleted,
        data: {'processed_count': processedCount},
      );

  factory MemorySyncCompletedMessage.fromJson(Map<String, dynamic> json) {
    final data = json['data'] as Map<String, dynamic>? ?? {};
    return MemorySyncCompletedMessage(
