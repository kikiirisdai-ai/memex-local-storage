...
import 'daos/card_embedding_dao.dart';
...
@DriftDatabase(
  ...
  daos: [CardDao, CardEmbeddingDao, GoalDao],
)
class AppDatabase extends _$AppDatabase {
  ...
  /// Set to true during onUpgrade when the card_embeddings table is first
  /// created for an existing user (schema v17). [MemexRouter._init] reads
  /// and clears this after services are ready so it can trigger a one-time
  /// full embedding backfill. Mirrors [_needsFtsRebuild].
  bool _needsEmbeddingRebuild = false;

  /// Whether a full embedding backfill is needed (set during migration,
  /// cleared after the backfill runs).
  bool get needsEmbeddingRebuild => _needsEmbeddingRebuild;

  /// Clear the embedding rebuild flag after the backfill has been triggered.
  void clearEmbeddingRebuildFlag() => _needsEmbeddingRebuild = false;
  ...
  /// Single source of truth for the current schema version. Kept in sync
  /// with the [schemaVersion] getter to prevent drift.
  ...
  MigrationStrategy get migration => MigrationStrategy(
        onCreate: (Migrator m) async {
          ...
          await customStatement(
            'CREATE INDEX IF NOT EXISTS idx_card_embeddings_fact_id ON card_embeddings(fact_id)',
          );
        ...
        onUpgrade: (Migrator m, int from, int to) async {
          ...
          if (from < 17) {
            // Guard against a residual table (e.g. DB file surviving a
            // reinstall): createTable throws "table already exists", which
            // would abort onUpgrade and hang DB init → white screen.
            try {
              await m.createTable(cardEmbeddings);
            } catch (e) {
              _logger.info(
                  'card_embeddings may already exist, skipping create: $e');
            }
            await customStatement(
              'CREATE INDEX IF NOT EXISTS idx_card_embeddings_fact_id ON card_embeddings(fact_id)',
            );
            // Existing users upgrading to v17 need a full embedding backfill
            // because their cards were never embedded. We set a flag here
            // and MemexRouter._init() picks it up after all services are
            // ready. Mirrors the FTS rebuild flag (from < 10 above).
            _needsEmbeddingRebuild = true;
          }
          if (from < 18) {
            // Guard against a residual table (mirrors the v17 CardEmbeddings
            // migration): createTable throws "table already exists", which
            // would abort onUpgrade and hang DB init → white screen.
            try {
              await m.createTable(goals);
            } catch (e) {
              _logger.info('goals table may already exist, skipping create: $e');
            }
            await customStatement(
              'CREATE INDEX IF NOT EXISTS idx_goals_user_status ON goals(user_id, status)',
            );
