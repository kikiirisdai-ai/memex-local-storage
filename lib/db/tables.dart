...
/// Stores vector embeddings for card content, used for semantic search.
class CardEmbeddings extends Table {
  TextColumn get factId => text()(); // Primary Key: yyyy/mm/dd.md#ts_n
  IntColumn get dim => integer()(); // Vector dimensionality
  TextColumn get vector => text()(); // JSON-encoded List<double>
  TextColumn get model => text()(); // Embedding model identifier
...
/// Goals Table
/// Standalone goal tracking, not rendered on the timeline. Read by quick
/// capture to offer goal-progress suggestions; progress is written when the
/// user confirms a suggestion or edits in the goals page. Otherwise manual
...
class Goals extends Table {
  ...
  TextColumn get userId => text()();
  TextColumn get title => text()();
  TextColumn get goalType => text()(); // 'quantitative' | 'binary'
  RealColumn get targetValue => real().nullable()();
  RealColumn get currentValue => real().withDefault(const Constant(0))();
  TextColumn get unit => text().nullable()();
  IntColumn get deadline => integer().nullable()(); // unix seconds
