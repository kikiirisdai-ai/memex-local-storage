...
class _SettingsPageState extends State<SettingsPage> {
  static final _wipeLogger = getLogger('SettingsPage.wipe');
  final ExportBackupViewModel _exportBackupVm = ExportBackupViewModel();
  ...
  void initState() {
    super.initState();
    _loadSettings();
    _exportBackupVm.loadLastExport();
  }

  @override
  void dispose() {
    _exportBackupVm.dispose();
    super.dispose();
  }
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
            ...
            child: InkWell(
              onTap: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => MemoryBookExportPage(
                      viewModel: MemoryBookExportViewModel(
          ...
          const SizedBox(height: 16),
          // Export Backup — prominent one-tap share of a .memex.
          // The only backup that survives an uninstall on this build lives
          // OUTSIDE the app sandbox, so make exporting easy to reach.
          ExportBackupTile(viewModel: _exportBackupVm),
          const SizedBox(height: 16),
          // Backup & Restore
          Material(
            color: Colors.transparent,
  ...
  /// Resolves a directory to write the pre-wipe safety snapshot into,
  /// guaranteed (with a defensive fallback) to be outside
  /// [FileSystemService.instance.dataRoot] — which is about to be
  /// recursively deleted by the wipe. See [takeWipeSafetySnapshot].
  ///
  /// Delegates to [BackupService.resolveSafetySnapshotDirectory] so the
  /// write path here and the list path in
  /// [BackupService.listStoredBackups] can never drift apart.
  Future<String> _resolveWipeSafetySnapshotDir() async {
  ...
  Future<void> _showDeleteAccountDialog() async {
            ...
            return AlertDialog(
              ...
              content: Column(
                ...
                children: [
                  ...
                  Text(
                    l10n.deleteAccountSafetySnapshotNote,
    ...
    try {
      ...
      // 2.5. Best-effort safety snapshot before the destructive wipe below.
      // MUST be written outside FileSystemService.instance.dataRoot — step 3
      // recursively deletes dataRoot, and (in the default StorageLocation.app
      // config) the "default backup directory" used elsewhere in this file
      // lives *inside* dataRoot, which would destroy the snapshot along with
      // everything else. getApplicationSupportDirectory() is a sibling of the
      // documents directory dataRoot points to, so it survives the deletion.
      await takeWipeSafetySnapshot(
        resolveSafeDir: _resolveWipeSafetySnapshotDir,
        createBackup: (outputDir) => BackupService.createBackup(
          outputDirectory: outputDir,
          filePrefix: 'memex_safety_before_wipe',
        ),
        onError: (e) {
          _wipeLogger.warning(
            'Safety snapshot before wipe failed (non-blocking): $e',
          );
        },
      );
