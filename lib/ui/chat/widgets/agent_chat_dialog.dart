...
class AgentChatDialog extends StatefulWidget {
  ...
  /// Voice-note WAV path accompanying [initialDraftText] (the transcription).
  /// Sent along with the next message so the timeline card keeps the audio.
...
class _AgentChatDialogState extends State<AgentChatDialog>
  ...
  /// Voice-note audio waiting to ride along with the next sent message.
  String? _pendingAudioPath;

  // In-dialog voice input: tap mic to record, tap again to stop; the
  // transcript lands in the input field and the audio rides with the send.
  final AudioRecorder _voiceRecorder = AudioRecorder();
  StreamSubscription<Uint8List>? _voiceAudioSub;
  final List<int> _voicePcmBuffer = [];
  bool _isVoiceRecording = false;
  bool _isVoiceTranscribing = false;
  StreamingTranscriber? _voiceStreamingTranscriber;

  /// Input text present before recording started; the live transcript is
  /// appended after it while recording.
  String _voiceBaseText = '';
  bool _isFullScreen = false;
  final AgentRunMode _runMode = AgentRunMode.auto;
  ...
  /// Guards against overlapping [_handlePolish] calls (e.g. a fast double
  /// tap) which would otherwise stack compare dialogs and risk duplicate
  /// cards.
  ...
  void initState() {
    ...
    _pendingAudioPath = widget.initialAudioPath;
  ...
  /// Sends the current input text under [runMode] (used by the create-card
  /// and AI-interact action buttons). Submission now happens exclusively
  /// through the three [InputActionButtons] — there is no standalone send
  /// button — so this also carries the onboarding-demo and
  /// notification-permission side effects that used to live on the removed
  /// send arrow's tap handler.
  void _sendCurrent({required String runMode}) {
  ...
  /// Polishes the current input text and, if the user confirms a rewrite,
  /// sends it as a card with the polish metadata attached.
  Future<void> _handlePolish() async {
    if (_isPolishing) return; // Re-entrancy guard: no stacked polish flows.
    final text = _messageController.text.trim();
    if (text.isEmpty) return;

    setState(() => _isPolishing = true);
    try {
      final result = await PolishService.instance.polish(text);
      if (!mounted) return;

      if (!result.anySucceeded) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(UserStorage.l10n.modelOfflineToast)),
        );
        return; // Original text stays in the input.
      }

      final choice = await showPolishCompareDialog(
        context,
        original: text,
        result: result,
      );
      ...
      final edited = await showPolishEditConfirmSheet(
        context,
        initialText: choice.text,
      );
      if (edited == null || edited.isEmpty || !mounted) return;

      // _sendMessage clears the input (and any attached images) itself, so
      // no separate _messageController.clear() is needed here.
      _sendMessage(
        edited,
        ...
        runMode: 'card',
        originalText: text,
        polishedStyle: choice.style == 'original' ? null : choice.style,
        verbatimText: true,
      );
    } finally {
      if (mounted) {
            ...
            child: Column(
              ...
              children: [
                ...
                InputActionButtons(
                  ...
                  onCreateCard: () => _sendCurrent(runMode: 'card'),
                  onAiInteract: () => _sendCurrent(runMode: 'agent'),
                  onPolish: _handlePolish,
                  isPolishing: _isPolishing,
                ),
                const SizedBox(height: 8),
                Row(
                  children: [
                    ...
                    const SizedBox(width: 12),
                    _buildInputIconButton(
                      key: const ValueKey('super_agent_mic_button'),
                      icon: _isVoiceTranscribing
                          ? Icons.hourglass_top
  ...
  Future<void> _toggleVoiceInput() async {
    if (_isVoiceTranscribing) return;

    if (_isVoiceRecording) {
      // Stop → transcribe → fill input + attach audio.
      setState(() {
      ...
      try {
        ...
        _voiceStreamingTranscriber?.dispose();
        _voiceStreamingTranscriber = null;

        if (_voicePcmBuffer.isNotEmpty) {
          final aligned = Uint8List.fromList(_voicePcmBuffer);
          ...
          final transcript = await SpeechTranscriptionService.instance
              .transcribeSamples(samples);

          final directory = await getTemporaryDirectory();
          final timestamp = DateTime.now().millisecondsSinceEpoch;
          final wavPath = '${directory.path}/dialog_audio_$timestamp.wav';
          await SpeechTranscriptionService.instance.savePcmAsWav(
            wavPath,
            Uint8List.fromList(_voicePcmBuffer),
          );
          _voicePcmBuffer.clear();

          if (transcript != null && transcript.trim().isNotEmpty) {
            _pendingAudioPath = wavPath;
            // Replace the live-preview text with the final calibrated
            // transcription (more accurate than the streaming preview).
            final base = _voiceBaseText.trim().isEmpty
                ? ''
                : '${_voiceBaseText.trimRight()}\n';
            final merged = '$base${transcript.trim()}';
        ...
        _logger.severe('In-dialog voice input failed: $e');
      } finally {
        if (mounted) setState(() => _isVoiceTranscribing = false);
      }
      return;
    }

    // Start recording.
    if (await SpeechTranscriptionService.instance
        .requiresLocalModelDownload()) {
      if (mounted) {
        ToastHelper.showInfo(context, UserStorage.l10n.speechNoResult);
      }
      return;
    ...
    try {
      ...
      // Live preview: stream the transcript into the input field while
      // recording so the user can see their speech being captured.
      if (await SpeechTranscriptionService.instance
          .supportsStreamingTranscription()) {
        _voiceStreamingTranscriber = StreamingTranscriber(
          onTextChanged: (fullText) {
            if (!mounted || !_isVoiceRecording) return;
            final base = _voiceBaseText.trim().isEmpty
                ? ''
                : '${_voiceBaseText.trimRight()}\n';
            final merged = '$base$fullText';
            _messageController.text = merged;
            _messageController.selection =
                TextSelection.collapsed(offset: merged.length);
          },
        );
        await _voiceStreamingTranscriber!.init();
      }
      ...
      _voiceAudioSub = audioStream.listen((chunk) {
        _voicePcmBuffer.addAll(chunk);
        _voiceStreamingTranscriber?.addAudioChunk(chunk);
      });
      setState(() => _isVoiceRecording = true);
    } catch (e) {
      _logger.severe('Failed to start in-dialog voice recording: $e');
    }
  }
  ...
  Widget _buildArtifactItem(ArtifactItem item, {bool embedded = false}) {
    ...
    if (artifact.kind == ChatArtifact.kindGoalSuggestion) {
      final card = GoalSuggestionArtifactCard(artifact: artifact);
      return embedded
