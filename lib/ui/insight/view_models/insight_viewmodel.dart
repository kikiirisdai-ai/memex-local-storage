...
class InsightViewModel extends ChangeNotifier {
  ...
  InsightSection selectedSection = InsightSection.stats;
  bool isLoading = true;
  String? errorMessage;
  String? activeCardId;
  ...
  Future<void> loadData() async {
    // The AI knowledge-insight generation path (fetchKnowledgeInsights) is
    // de-wired: it never completes on the local model (times out / returns
    // empty / hallucinates success with 0 real tool calls). The Insight page
    // now shows only Activity Stats, so loadData just delegates to loadStats.
    await loadStats();
  }
