...
/// (`isAi == false`) are always kept. Extracted as a pure function so it is
...
class _TimelineCardDetailScreenState extends State<TimelineCardDetailScreen> {
  ...
  Future<void> _loadUserInfo() async {
    ...
    if (mounted) {
      setState(() {
        ...
        _commentSettings = settings;
      });
    }
  }

  /// Comments visible in the UI, honoring [CommentSettings.enableCharacterComment].
  /// reversible: toggling the setting back on restores all comments.
  List<Comment> _visibleComments(CardDetailModel detail) {
    return visibleComments(
      detail.insight.comments,
      _commentSettings.enableCharacterComment,
    );
  }
  ...
  String _buildCardReferenceContent(CardDetailModel detail) {
    ...
    final shareableComments = _visibleComments(detail);
    if (shareableComments.isNotEmpty) {
      buffer.writeln('Comments:');
      for (final comment in shareableComments) {
        final authorName =
  ...
  Future<void> _editMood() async {
    ...
    final score = await showModalBottomSheet<int>(
      ...
      builder: (context) => MoodScorePicker(currentScore: _detail!.moodScore),
    );
    ...
    final oldDetail = _detail;
    setState(() {
      _detail = _detail!.copyWith(metadata: {
        ...?_detail!.metadata,
        CardMetadataKeys.userMoodScore: score,
      });
    });

    final ok = await _memexRouter.updateCardMood(widget.cardId, score);
    if (!mounted) return;
    if (ok) {
      ...
      ToastHelper.showError(
        context,
        UserStorage.l10n.updateFailed('mood'),
      );
    }
  }

  Future<void> _editMediaStatus() async {
    if (_detail == null) return;
    ...
    final status = await showModalBottomSheet<String>(
      ...
      builder: (context) =>
          MediaStatusPicker(currentStatus: _detail!.mediaStatusValue),
    );
    ...
    setState(() {
      _detail = _detail!.copyWith(metadata: {
        ...?_detail!.metadata,
        CardMetadataKeys.mediaStatus: status,
      });
    });

    final ok = await _memexRouter.updateCardMediaStatus(widget.cardId, status);
    if (!mounted) return;
    if (ok) {
      ToastHelper.showSuccess(context, UserStorage.l10n.saved);
    } else {
  ...
  Future<void> _shareCard() async {
    if (_detail == null) return;
    try {
      await _shareCardInner();
    } catch (e, st) {
      // Poster rendering (captureFromWidget) can fail on device; surface it
  ...
  Future<void> _shareCardInner() async {
  ...
  Widget _buildShareCommentsList(CardDetailModel detail) {
    ...
    // Other comments
    for (var comment in _visibleComments(detail)) {
                                  ...
                                  if (detail.mediaType != null) ...[
                                    Row(
                                      children: [
                                        MediaStatusBadge(
                                          detail: detail,
                                          onTap: _editMediaStatus,
                                        ),
                                        const SizedBox(width: 8),
                                        MediaRatingBadge(
                                          detail: detail,
                                          onTap: _editMediaRating,
                                        ),
                                      ],
                                    ),
                                    const SizedBox(height: 12),
  ...
  Widget _buildCommentsList(CardDetailModel detail) {
    ...
    final visible = _visibleComments(detail);
    for (var comment in visible) {
      final isUser = !comment.isAi;
  ...
  Widget _buildSingleComment({
    ...
    final bool isCharacterAvatar = !isUserComment &&
        ...
        characterId != null &&
        characterId != '0';
    final tappableAvatar = (isCharacterAvatar && kShowPersonaChat)
        ? GestureDetector(
            onTap: () {
              Navigator.push(
                context,
...
class TimelineCardDetailBodySection extends StatefulWidget {
  ...
  @override
  State<TimelineCardDetailBodySection> createState() =>
      _TimelineCardDetailBodySectionState();
}

class _TimelineCardDetailBodySectionState
    extends State<TimelineCardDetailBodySection> {
  ...
  Widget build(BuildContext context) {
    ...
    final bodyText =
        detail.originalText != null && _showOriginal
            ? detail.originalText!
            : detail.rawContent;
      ...
      children: [
          ...
          child: Text.rich(
            TextSpan(
              children: [
                ...
                if (bodyText.isNotEmpty && detail.tags.isNotEmpty)
                  const TextSpan(text: ' '),
                ...detail.tags.map((tag) {
                  return TextSpan(
                    text: '#$tag',
        ...
        if (detail.originalText != null)
          Align(
            ...
            child: TextButton(
              key: const ValueKey('card_original_toggle'),
              onPressed: () => setState(() => _showOriginal = !_showOriginal),
              child: Text(
                _showOriginal
                    ? UserStorage.l10n.showPolished
                    : UserStorage.l10n.showOriginal,
              ),
            ),
          ),
      ],
