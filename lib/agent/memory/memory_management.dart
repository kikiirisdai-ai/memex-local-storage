...
/// How similar two memory strings need to be (character-bigram Jaccard
/// similarity, 0-1) to treat the newer one as a near-duplicate of the
/// older one and skip adding it. Operates on character n-grams rather
/// than whitespace-delimited words so it works for CJK text (no spaces
/// between words) as well as English. Bigrams (rather than trigrams)
/// tolerate the word-order shuffling common in CJK paraphrases (e.g.
/// "喜欢周末去爬山" vs "周末喜欢去爬山") while still being specific
/// enough not to match unrelated sentences.
const double _nearDuplicateSimilarityThreshold = 0.5;

/// Whether [candidate] is similar enough to [existing] to be treated as a
/// near-duplicate (including but not limited to exact matches).
@visibleForTesting
bool isNearDuplicateMemory(String candidate, String existing) {
  return _charBigramSimilarity(candidate, existing) >=
      _nearDuplicateSimilarityThreshold;
}
...
double _charBigramSimilarity(String a, String b) {
  ...
  final gramsA = _charBigrams(normalizedA);
  final gramsB = _charBigrams(normalizedB);
...
Set<String> _charBigrams(String normalized) {
  final collapsed = normalized.replaceAll(RegExp(r'\s+'), ' ');
  if (collapsed.length < 2) return {collapsed};

  final grams = <String>{};
  for (var i = 0; i <= collapsed.length - 2; i++) {
    grams.add(collapsed.substring(i, i + 2));
  }
  return grams;
}
...
class MemoryManagement {
  ...
  Future<String> appendMemories(List<String> memories) async {
    return _runWithMemoryLock(() async {
      ...
      // Content already in the buffer (plus anything newly added within
      // this same call), for near-duplicate detection below.
      final existingContents = buffer
      ...
      for (final memory in memories) {
        ...
        final isDuplicate = existingContents
            .any((existing) => isNearDuplicateMemory(m, existing));
        if (isDuplicate) {
          skippedDuplicates++;
          continue;
        }
        existingContents.add(m);
      ...
      String resultMsg = addedIds.isEmpty
          ? "No new memories added (all were duplicates already in the recent buffer)."
          : "Memories appended successfully. IDs: ${addedIds.join(', ')}";
      if (skippedDuplicates > 0 && addedIds.isNotEmpty) {
        resultMsg += "\n(Skipped $skippedDuplicates duplicate(s) already in the recent buffer)";
      }
  ...
  /// Forces the recent buffer to consolidate into the archived profile
  /// right now, regardless of [recentBufferThreshold]. Used by the manual
  /// "force archive" action in the Memory screen. Returns false (no-op)
  /// when the buffer is already empty.
  Future<bool> forceConsolidate() async {
    return _runWithMemoryLock(() async {
      var mem = await _loadMemory();
      final buffer = (mem['recent_buffer'] as List?) ?? [];
