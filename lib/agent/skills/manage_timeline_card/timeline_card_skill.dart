      ...
      Tool(
        ...
        executable: (String? fact_id,
          ...
          try {
            ...
            // A `media_card` ui_config carries its media fields (type,
            // status, rating, comment) only in its own `data` map. The
            // detail-page media badges read those fields from the card's
            // top-level `metadata` instead, so mirror them there whenever
            // this call writes a `media_card` config — otherwise the badges
            // never appear for cards created/edited through this tool.
            UiConfig? mediaConfig;
            for (final config in uiConfigEntries ?? const <UiConfig>[]) {
              if (config.templateId == 'media_card') {
                mediaConfig = config;
                break;
              }
            }
            ...
            final updatedCardData = await fileService.updateCardFile(
              ...
              (card) {
                ...
                if (mediaConfig != null) {
                  final derived =
                      mediaMetadataFromUiConfigData(mediaConfig.data);
                  if (derived.isNotEmpty) {
                    metadataOverride = {...?card.metadata, ...derived};
                  }
                }
                var c = card.copyWith(
                  status: 'completed',
