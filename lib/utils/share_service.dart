      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setDialogState) => SharePreviewDialog(
          ...
          onShare: () async {
            ...
            try {
              await _performShare(context, currentBytes);
            } catch (e) {
              // This callback runs detached from shareWidgetAsPoster's own
              // await chain (the dialog already popped), so a throw here
              // would otherwise become a silently-unhandled async error —
              // the caller's try/catch around shareWidgetAsPoster() never
              // sees it. Surface it here instead.
              if (context.mounted) {
