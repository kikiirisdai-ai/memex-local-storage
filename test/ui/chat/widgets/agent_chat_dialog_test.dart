...
void main() {
  ...
  group('AgentChatDialog full-screen affordance', () {
    ...
    testWidgets(
        'super agent entry hides chat controls, uses send input, and '
        'submits only via the action buttons (no send arrow)', (
      tester,
    ) async {
      await _pumpDialog(tester);
      ...
      expect(
        find.byKey(const ValueKey('super_agent_mic_button')),
        findsOneWidget,
      );
      // The send arrow is gone: submission happens only via the three
      // InputActionButtons (创建卡片/AI互动/润色).
      expect(
        find.byKey(const ValueKey('super_agent_publish_button')),
        findsNothing,
      );
      expect(find.text(UserStorage.l10n.sendLabel), findsNothing);
      expect(
      ...
      expect(find.byKey(const ValueKey('action_polish')), findsOneWidget);
    });

    testWidgets(
        'renders a completed artifact card but does not open schedule tab '
        'while kShowScheduleEntry is false', (
      tester,
    ) async {
      // reversible: kShowScheduleEntry (lib/ui/timeline/widgets/timeline_screen.dart)
      // gates schedule navigation off the schedule chat artifact. Flipping it
      // back to true restores the tap-to-open behavior this test used to cover.
    ...
    testWidgets(
        'labels search_cards source artifacts as "source card", not '
        '"card created"', (tester) async {
      await _pumpDialog(
        tester,
        dialog: AgentChatDialog(
          initialItems: [
            AIMessageItem(
              'Last week you went running twice.',
              artifacts: [
                ArtifactItem(
                  ChatArtifact.timelineCard(
                    cardId: '2026/08/01.md#ts_1',
                    title: 'Morning run',
                    summary: 'Ran 5km',
                    updated: false,
      ...
      expect(find.text(UserStorage.l10n.agentChat.cardSource), findsOneWidget);
      expect(
        find.text(UserStorage.l10n.agentChat.cardCreated),
        findsNothing,
      );
