...
void main() {
  ...
  testWidgets('shows activity stats page directly (AI insights removed)', (
    tester,
  ) async {
  ...
  testWidgets(
    'metric tiles do not overflow at a larger system text scale',
    (tester) async {
      final vm = _buildViewModel();

      await tester.pumpWidget(
        _wrap(
          InsightScreen(viewModel: vm, isEmbedded: true),
          // Simulates a real device with a larger accessibility/system
          // font size — the fixed childAspectRatio grid cells overflowed
          // by a constant amount on every tile at this scale (real-device
          // bug, never caught at the default 1.0 scale used elsewhere in
          // this file).
          textScaleFactor: 1.3,
        ),
      );
  ...
  testWidgets('reloads fresh stats via loadData (no AI insight fetch)', (
    tester,
  ) async {
    ...
    // loadData() (called by the app on startup) now just delegates to
    // loadStats() — the AI knowledge-insight fetch is de-wired.
    await vm.loadData();
    await tester.pump(const Duration(milliseconds: 300));
    ...
    expect(
      find.text(
        'In this period you recorded 1 time(s) and generated 0 card(s).',
      ),
      findsOneWidget,
    );
  ...
  testWidgets('refreshes stats page when activity changes elsewhere', (
    tester,
  ) async {
  ...
  testWidgets('switches metric and opens day detail sheet', (tester) async {
    ...
    await tester.ensureVisible(find.byKey(const ValueKey('stats_metric_cards')));
    await tester.pump();
  ...
  testWidgets('renders mood curve chart when some days have mood data', (
    tester,
  ) async {
    await tester.pumpWidget(
      _wrap(
        UserStatsPage(
          snapshot: _snapshotWithMood(),
          isLoading: false,
          errorMessage: null,
          selectedDays: 3,
          selectedMetric: UserStatsMetric.inputs,
    ...
    expect(find.byKey(const ValueKey('mood_curve_chart')), findsOneWidget);
    expect(find.byType(LineChart), findsWidgets);
    expect(find.byKey(const ValueKey('mood_curve_empty')), findsNothing);
  });

  testWidgets('shows empty state when no day has mood data', (tester) async {
    await tester.pumpWidget(
      _wrap(
        UserStatsPage(
          snapshot: _snapshot(),
    ...
    expect(find.text('Mood curve'), findsOneWidget);
    expect(find.byKey(const ValueKey('mood_curve_empty')), findsOneWidget);
    expect(find.byKey(const ValueKey('mood_curve_chart')), findsNothing);
  });
...
UserStatsSnapshot _snapshotWithMood() {
  final base = _snapshot();
  final daily = [
    UserStatsDailyPoint(
      date: base.daily[0].date,
      inputs: base.daily[0].inputs,
      words: base.daily[0].words,
      cards: base.daily[0].cards,
      knowledgeUnits: base.daily[0].knowledgeUnits,
      insights: base.daily[0].insights,
      completedTodos: base.daily[0].completedTodos,
      moodAverage: 7.0,
    ),
    UserStatsDailyPoint(
      date: base.daily[1].date,
      inputs: base.daily[1].inputs,
      words: base.daily[1].words,
      cards: base.daily[1].cards,
      knowledgeUnits: base.daily[1].knowledgeUnits,
      insights: base.daily[1].insights,
      completedTodos: base.daily[1].completedTodos,
      moodAverage: null,
    ),
    UserStatsDailyPoint(
      date: base.daily[2].date,
      inputs: base.daily[2].inputs,
      words: base.daily[2].words,
      cards: base.daily[2].cards,
      knowledgeUnits: base.daily[2].knowledgeUnits,
      insights: base.daily[2].insights,
      completedTodos: base.daily[2].completedTodos,
      moodAverage: 4.5,
    ),
  ];
  return UserStatsSnapshot(
    ...
    daily: daily,
    sourceBreakdown: base.sourceBreakdown,
