...
void main() {
  ...
  test(
    'computes deterministic daily mood average, user override wins, no mood is a gap',
    () async {
      final fs = FileSystemService.instance;

      // Day 1: two cards, one AI-scored, one user-overridden -> average of
      // the *effective* scores (user override wins over AI on its card).
      await fs.safeWriteCardFile(
        userId,
        '2026/06/01.md#ts_0',
        CardData(
          factId: '2026/06/01.md#ts_0',
          timestamp: DateTime(2026, 6, 1, 8).millisecondsSinceEpoch ~/ 1000,
          status: 'completed',
          tags: const [],
          uiConfigs: const [],
          metadata: const {CardMetadataKeys.moodScore: 8},
        ),
      );
      await fs.safeWriteCardFile(
        userId,
        '2026/06/01.md#ts_1',
        CardData(
          factId: '2026/06/01.md#ts_1',
          timestamp: DateTime(2026, 6, 1, 9).millisecondsSinceEpoch ~/ 1000,
          status: 'completed',
          tags: const [],
          uiConfigs: const [],
          metadata: const {
            CardMetadataKeys.moodScore: 9,
            CardMetadataKeys.userMoodScore: 6,
          },
        ),
      );

      // Day 2: no mood data at all -> must stay a gap (null), not 0.
      await fs.safeWriteCardFile(
        userId,
        '2026/06/02.md#ts_0',
        CardData(
          factId: '2026/06/02.md#ts_0',
          timestamp: DateTime(2026, 6, 2, 8).millisecondsSinceEpoch ~/ 1000,
          status: 'completed',
          tags: const [],
          uiConfigs: const [],
          fact: 'No mood recorded here',
        ),
      );
      ...
      final day1 = snapshot.pointFor(DateTime(2026, 6, 1))!;
      // effective scores: [8 (no override), 6 (user override wins)] -> 7.0
      expect(day1.moodAverage, 7.0);

      final day2 = snapshot.pointFor(DateTime(2026, 6, 2))!;
      expect(day2.moodAverage, isNull);
    },
  );
