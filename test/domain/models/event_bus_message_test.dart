...
void main() {
  ...
  test('parses memory sync completed messages from event payloads', () {
    final parsed = EventBusMessage.fromJson({
      'type': 'memory_sync_completed',
      'data': {'processed_count': 5},
    });

    expect(parsed, isA<MemorySyncCompletedMessage>());
    final message = parsed as MemorySyncCompletedMessage;
    expect(message.type, EventBusMessageType.memorySyncCompleted);
    expect(message.processedCount, 5);
  });

  test('memory sync completed message round-trips through toJson', () {
    final message = MemorySyncCompletedMessage(processedCount: 3);

    expect(message.data, {'processed_count': 3});
  });
}
